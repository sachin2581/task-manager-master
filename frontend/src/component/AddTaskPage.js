...
export default function TaskPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center bg-gray-50 px-4 py-10">
      {/* Form */}
      <div className="w-full max-w-4xl p-6 bg-white rounded-2xl shadow-md mb-10">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <input
            ...
            required
            placeholder="Title"
            className="px-3 py-2 border border-gray-300 rounded-lg focus:ring focus:ring-blue-300"
          />
          <input
            type="date"
            name="dueDate"
          ...
          <textarea
            ...
            placeholder="Description"
            rows={2}
            className="col-span-1 md:col-span-2 px-3 py-2 border border-gray-300 rounded-lg focus:ring focus:ring-blue-300"
          />
          <select
            name="status"
            value={task.status}
          ...
          <button
            type="submit"
            disabled={loading}
            className="bg-blue-600 text-white flex items-center justify-center gap-2 px-5 py-2 rounded-lg hover:bg-blue-700 transition disabled:opacity-50 col-span-1 md:col-span-2"
          >
            {loading ? "Saving..." : editId ? <><FaEdit /> Update Task</> : <><FaPlus /> Add Task</>}
          </button>
        </form>
      </div>
      ...
      <div className="w-full max-w-4xl">
        ...
        <div className="grid gap-4">
          {tasks.map((t) => (
            <div
              key={t._id}
              className="bg-white shadow-md rounded-xl p-5 flex flex-col md:flex-row md:items-center justify-between border border-gray-100"
            >
              <div className="flex-1">
                ...
                <div className="flex gap-3 items-center text-sm text-gray-500">
                  ...
                  <span>📅 {new Date(t.dueDate).toLocaleDateString("en-GB")}</span>
                </div>
              </div>
